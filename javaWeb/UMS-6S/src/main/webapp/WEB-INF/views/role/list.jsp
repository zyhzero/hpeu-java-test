...
<form action="" id="deleteUserForm">
	<table class="table table-bordered table-hover">
		<thead>
			<tr>
				<th colspan="5">
					<button class="btn btn-info" id="save-role" type="button">添加</button>
					<button class="btn btn-danger remove-role" type="button">删除</button>
				</th>
			</tr>
		</thead>
		<tbody>
			...
			<c:forEach var="item" items="${listRole }" varStatus="status">
				<tr class="item-list" data-id="${item.id }">
					...
					<td class="item-menu-group text-right"><a
						...
						data-id="${item.id }"><i class="iconfont icon-edit2"></i></a> <a
...
<script>
	$(function() {
		...
		$("#save-role").on("click", function() {
			$(".content_panel").load("${pageContext.request.contextPath }/role/add");
		});

		//编辑按钮
		$(".eidt-Role").on("click", function() {
			var id = $(this).data("id");
			$(".content_panel").load("${pageContext.request.contextPath }/role/edit/"+id);
		});
		...
		$(".removeRole").on("click", function() {
			if (confirm("确认删除？")) {
				var $this = $(this);
				var id = $this.data("id");
				$.get("${pageContext.request.contextPath }/role/delete/"+id,function(data) {
					if(data.success){
						toastr.success("删除成功！");
		...
		$(".remove-role").on("click", function() {
			...
			$.post("${pageContext.request.contextPath }/role/delete",$('#deleteUserForm').serialize(),function(data){	
				if(data.success){
					toastr.success("删除成功！");	
					$(".content_panel").load("${pageContext.request.contextPath }/role/list");
				}else{
		...
		$(".firstPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/role/list?pageNow=1");
		});
		$(".prePage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/role/list?pageNow=${page.pageNow-1}");
		});
		$(".nextPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/role/list?pageNow=${page.pageNow+1}");
		});
		$(".lastPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/role/list?pageNow=${page.totalPage}");
		});
	});
</script>