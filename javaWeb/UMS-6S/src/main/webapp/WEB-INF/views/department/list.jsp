...
<form action="" id="deleteDepartmentForm">
	<table class="table table-bordered table-hover">
		...
		<tr>
			<th colspan="5">
				<button class="btn btn-info" id="save-department" type="button">添加</button>
				<button class="btn btn-danger remove-department" type="button">删除</button>
			</th>
		</tr>
		</thead>
		<tbody>
		...
		<c:forEach var="item" items="${listDepartment }" varStatus="status">
			<tr class="item-list" data-id="${item.id }">
				...
				<td class="item-menu-group text-right"><a class="btn btn-sm btn-info eidt-Department"
					data-id="${item.id }" href="javascript:;"><i class="iconfont icon-edit2"></i></a> <a
...
<script>
	$(function() {
		...
		$("#save-department").on("click", function() {
			$(".content_panel").load("${pageContext.request.contextPath }/department/add");
		});

		//编辑按钮
		$(".eidt-Department").on("click", function() {
			var id = $(this).data("id");
			$(".content_panel").load("${pageContext.request.contextPath }/department/edit/"+id);
		});
		...
		$(".removeDepartment").on("click", function() {
			if (confirm("确认删除？")) {
				...
				$.get("${pageContext.request.contextPath }/department/delete/"+id,function(data) {
					if(data.success){
	                    toastr.success("删除成功！");
		...
		$(".remove-department").on("click", function() {
			...
			$.post("${pageContext.request.contextPath }/department/delete",$('#deleteDepartmentForm').serialize(),function(data){	
				if(data.success){
					toastr.success("删除成功！");	
					$(".content_panel").load("${pageContext.request.contextPath }/department/list");
				}else{
		...
		$(".firstPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/department/list?pageNow=1");
		});
		$(".prePage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/department/list?pageNow=${page.pageNow-1}");
		});
		$(".nextPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/department/list?pageNow=${page.pageNow+1}");
		});
		$(".lastPage").on("click", function() {
			$("#table").load("${pageContext.request.contextPath }/department/list?pageNow=${page.totalPage}");
		});
	});
</script>